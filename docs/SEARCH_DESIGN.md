...
1. **Default label:** icon-only or icon+"Search" text as the shipped default? (NN/g leans visible label; the pill layout argues icon-only. Spec assumes icon-only default with the label one toggle away.)
