...
# Check right-to-left siblings (generated by scripts/build-rtl.js in npm run build)
for rtl in style assets/css/header.min assets/css/header-breakpoint.min assets/css/footer.min assets/css/search.min assets/css/archive.min assets/css/woocommerce.min assets/css/breadcrumbs.min assets/css/announcement-bar assets/css/editor-style; do
    if [ ! -f "${rtl}-rtl.css" ]; then
        echo "  ❌ Missing: ${rtl}-rtl.css (run npm run build)"
        MISSING_MIN=1
    else
        echo "  ✓ ${rtl}-rtl.css"
    fi
